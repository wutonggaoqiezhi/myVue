...
export default class LoadManager extends THREE.EventDispatcher {
    ...
    handleRequests() {
        ...
        {
            this.loadQueue[i].noNeedForLoad() || this.handleRequest( this.loadQueue[i], ( message ) => { 
                ...
                this.handleRequests();
                this.dispatchEvent({type:"progress", data:{ loaded: this.numOfDone, total: this.numOfTotal }})
    ...
    private handleRequest( requset: LoadRequest, callBack: Function ) {
        ...
        let numOfNeedRequset = Math.min( this.loadQueue.length, this.loadWorkers.length );

        for( let i = 0; i < numOfNeedRequset; i++ )
        {
            let loadWorker = this.loadWorkers[i];
            if(!loadWorker.used)
            {
