...
export default class WuHouCiApp extends THREE.EventDispatcher {
    ...
    {
        let guidepost = this.glManager.hotspotController.guideposts.find( item => item.targetFlow == flow );
        ...
        this.glManager.openFlow( guidepost ).then(()=>{
            ...
            {
                let hotspot = this.glManager.hotspotController.hotspots.find(item => item.flow == flow);
    ...
    {
        let hotspot = this.glManager.findHotspotById( id );
        ...
        {
            let guidpost = this.glManager.hotspotController.guideposts.find( item => item.targetFlow == hotspot.flow);
