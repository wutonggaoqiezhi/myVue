...
export default class HotspotController {
    ...
    update( dt: number ) {
        ...
        this.hotspots.forEach( item => {   item.update( dt ); } );
        this.guideposts.forEach( item => {   item.update(dt ) });
    }
    ...
    openFlow( guidepost: Guidepost ) {
        ...
        let timer = setTimeout(()=>{
            this.guideposts.forEach( item => { item.visible = guidepost.hotspotId == item.hotspotId });
            clearTimeout(timer);
        },500)
    }
    ...
    {
        this.hotspots.forEach( item => { item.visible = item.flow == guidepost.targetFlow });
