...
import WuHouCiApp from 'src/WuHouCiApp';



class GLManager extends THREE.EventDispatcher {
    ...
    init() {
        ...
        let textureLoader = new THREE.TextureLoader().setPath("skybox/");

        ['px.jpg', 'nx.jpg', 'py.jpg', 'ny.jpg', 'pz.jpg', 'nz.jpg'].forEach((item)=>{
            materials.push(new THREE.MeshBasicMaterial({ map: textureLoader.load(item), side: THREE.BackSide }))
        })
    ...
    openFlow( guidepost: Guidepost ): Promise<any>
    {   
       
        let scene = this.panoSceneController.findSceneByFlow( guidepost.targetFlow );
        ...
        return new Promise((resolve, reject)=>{
            if(scene)
            {
                ...
                scene.mesh && scene.mesh.setVisible( true, 1000, 5000 );
                this.cameraController.moveToGuidepost( guidepost )
                .then(()=>{
    ...
    closeFlow( guidepost: Guidepost )
    {
        ...
        let panoScene = this.panoSceneController.findSceneByFlow(guidepost.targetFlow);
        panoScene && this.panoSceneController.outScene();
    ...
    findHotspotById( hotspotId ): Hotspot
    {
       return this.hotspotController.hotspots.find(item=>item.hotspotId == hotspotId );
    }
    ...
    private dispatchMouseEvent( event: MouseEvent  ) {
        ...
        let ndc = this.ndc;
        let mouseScreenPosition = this.mousePosition;
        ...
        this.scene.dispatchEvent( {type: event.type, data: { screenPosition: mouseScreenPosition , activeCamera: this.camera, raycaster: this.raycaster, ndc: ndc } });
    ...
    private dispatchTouchEvent( event: TouchEvent ) {
        ...
        ndc.set( (event.touches[0].clientX / window.innerWidth ) * 2 - 1, -( event.changedTouches[0].clientY / window.innerHeight ) * 2 + 1 );
