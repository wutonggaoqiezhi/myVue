...
export default class PanoSceneController extends THREE.EventDispatcher {
  ...
  init() {
    this.fileLoader.load('panoramas/panoScenes.json', (data: any) => {
      data.panoScenes.forEach((item: any) => {
        const panoscene = new PanoScene(this, item)
  ...
  reset() {
    if (this.activeScene) this.outScene()
  }
  ...
  updateMinimap(position: THREE.Vector3, direction: THREE.Vector3) {
    this.manager.app.dispatchEvent({
      type: 'minimap.update',
      data: { position: position, direction: direction },
    })
  }
  ...
  enterScene(scene: PanoScene): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      const entryPoint = scene.getEntryPoint()
      this.textureLoader.getTextureForPoint(entryPoint).then((texure) => {
        this.activeScene.onEnter(texure, entryPoint)

        const position = entryPoint.panoPosition.clone()
        this.activeScene.localToWorld(position)
        this.manager.transitions.add(
          new TWEEN.Tween(this.manager.camera.position)
  ...
  moveToDirection(direction?: THREE.Vector3) {
    ...
    if (this.activeScene && !this.isMoving) {
      const targetPoint = this.findClosestTargetInDirection(direction)
  ...
  findClosestTargetInDirection(direction: THREE.Vector3): PanoPoint | null {
    ...
    if (this.activeScene && this.activePoint) {
      point = this.activeScene.points.findRankedByScore(
        0,
        [
  ...
  moveToPoint(point: PanoPoint) {
    if (!this.isMoving) {
      this.isMoving = true
      this.textureLoader.getTextureForPoint(point).then((texture) => {
        const position = point.panoPosition.clone()
        this.activeScene.localToWorld(position)
        const duration = 750 + 150 * Math.min(this.manager.camera.position.distanceTo(position), 5)
  ...
  onMove(event: THREE.Event) {
    const raycaster = event.data as THREE.Raycaster
    const position = new THREE.Vector3()
    raycaster.ray.intersectPlane(this.activeScene.floor, position)
    this.activeScene.worldToLocal(position)
    const direction = position.sub(this.activePoint.panoPosition).normalize()
