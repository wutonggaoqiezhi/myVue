...
class PanoramaControls extends THREE.EventDispatcher {
    ...
    public bindEvents() {
        ...
        this.domElement.addEventListener("mouseover", (event: MouseEvent) => this.pointerDragOn && 0 === event.which && this.onMouseUp(event));
    ...
    public onMouseMove(event: MouseEvent) {
        ...
        if( Math.abs(event.clientX - this.pointerDownPosition.x) + Math.abs(event.clientY - this.pointerDownPosition.y) > 10 ) {
    ...
    public onTouchMove(event: TouchEvent) {
        ...
        if( Math.abs(event.touches[0].clientX - this.pointerDownPosition.x) + Math.abs(event.touches[0].clientY - this.pointerDownPosition.y) > 10 ) {
    ...
    public endRotation() {
        ...
        if (!(this.controller&&this.controller.isMoving)) {
            ...
            let historyLength = this.rotationHistory.length;
            var rotationHistoryAverage = this.rotationHistory.reduce((total, item) => total.add(item), new THREE.Vector2(0)).divideScalar(historyLength);
    ...
    public onMouseWheel(event: WheelEvent) {
        ...
        if (this.enabled) {
            ...
            let z = void 0 !== event['wheelDelta'] ? event['wheelDelta'] : void 0 !== event.detail && -event.detail;
