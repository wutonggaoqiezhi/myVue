<template>
    <div class="relic-details">
        ...
        <ul class="pre-list">
            <li v-for="(item,index) in resources" :key="index"  class="pre-item" @mouseover="tab(index)" @click="showDetail( true )"
                :style="{
...
export default {
    ...
    props: {
        ...
        resources: Array        //Resource.RelicDetails
    ...
    methods: {
        ...
        tab( index ) {
            ...
            console.log(Math.abs(this.activeIndex -index))
            this.activeIndex = index;

        },
        showDetail( visible ) {
            this.subList.show = visible;
        },
        subTab( step ) {

            let length = this.resources[this.activeIndex].images.length;
            this.subList.index = (this.subList.index  + length + step) % length;
