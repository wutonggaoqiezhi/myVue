<template>
    <div class="new-vs-old">
        ...
        <div class="main"  @mousemove="onMouseMove($event)" @touchmove="onTouchMove($event)" @mouseup="onMouseUp()" @touchend="onTouchEnd()">        
            <div class="drag" @mousedown="onMouseDown($event)" @touchstart="onTouchStart($event)" :style="{'left': `${dragState.x}px`}">
            ...
            <div class="main-item new" :style="{'background-image':`url(${resources[activeIndex].images.new})`, 'width': `${dragState.x}px`}" >
                <!-- <img :src="resources[activeIndex].images.new" /> -->
            </div> 
            <div class="main-item old" :style="{'background-image':`url(${resources[activeIndex].images.old})`, 'width': `${dragState.max-dragState.x}px`}" >
        ...
        <div class="preview">
            ...
            <div class="pre-viewport">
                <ul class="pre-list" :style="{'transform': `translateX(${-180*activeIndex}px)` }">
                    <li v-for=" (item, index) in resources" :key="index" class="pre-item" :class="{'active': activeIndex == index }"
                        :style="{'transform':`translateX(${ 1 }px)`,'background-image':`url(${item.preview})`}" @click="tab(index)" >
...
export default {
    ...
    data() {
        return {
            ...
            halfLength: Math.ceil(Resources.NewVsOld/2),
            dragState: {
                max: 600,
                x:600* 0.5,
    ...
    methods: {
        ...
        prev(  ) {
            ...
            if(this.activeIndex == length) return;
            
            this.activeIndex = (this.activeIndex+length+1) % length;
        ...
        onMouseDown( event ) {
            ...
            this.dragState.mouseMoveStartX = event.clientX;
            document.body.style.cursor = "e-resize"

        },
        onTouchStart( event ) {

            this.dragState.touch = true;
            //this.dragState.x = event.clientX;
            this.dragState.touchMoveStartX = event.touches[0].clientX;
        },
        onMouseMove( event ){

            if( this.dragState.mousedown )
            {   
                let movementX = event.clientX - this.dragState.mouseMoveStartX;
                this.dragState.x = Math.min( Math.max( this.dragState.x + movementX, 0 ), this.dragState.max  )
                this.dragState.mouseMoveStartX = event.clientX;
            }
        ...
        onTouchMove( event ) {
            ...
            if( this.dragState.touch ) {
                ...
                let movementX = event.touches[0].clientX - this.dragState.touchMoveStartX;
                this.dragState.x = Math.min( Math.max( this.dragState.x + movementX, 0 ), this.dragState.max  )
                this.dragState.touchMoveStartX = event.touches[0].clientX;
            }
        },
