    ...
    <div class="minmap" :style="{'background-image':'url('+map.src+')'}">
        <span class="curr" :style="{left:position.x+'px',top: position.y+'px'}">
            <span class="view-point" >
                <svg viewBox="0 0 50 50" class="view-range" :style="{color:'#fff',transform: `translate(-50%, -50%) rotateZ(${rotation}deg)`}">
...
export default {
    ...
    created() {
        ...
        this.map = this.flow == 7 ? maps[0]: maps[1];
    },
    mounted()
    {
    ...
    methods: {
        ...
        onUpdate(  data )
        {
            ...
            this.rotation = direction.x > 0 ? THREE.Math.radToDeg(direction.angleTo(forward) ): -THREE.Math.radToDeg(direction.angleTo(forward) )
            this.position = this.getTransform( position, this.flow )
            
        },
        getTransform( position, flow ) 
        {
            switch (flow) {
                case 7:
                    return {
...
.minmap {
    ...
    .curr {
        ...
        height: 7px;
        border-radius: 50%;
        transform: rotate(-135deg); /*校正角度使默认与相机世界方向一致*/
        z-index:10;
    }
